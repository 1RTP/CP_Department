...
internal sealed class ReportService : IReportService
{
    ...
    {
        using var stream = ResourceHelper.GetReportResource("nwind.xml");
    ...
    {
        using var reportStream = ResourceHelper.GetReportResource(name + REPORTS_EXTENSION);
    ...
    {
        using var reportStream = await ResourceHelper.GetReportResourceAsync(name + REPORTS_EXTENSION);
        var report = Report.FromStream(reportStream);
        RegisterData(report);
        return report;
    }
